...
interface Movie { id: number; title: string; slug: string; description?: string; shortDescription?: string; type: 'movie' | 'series'; year?: number; duration?: number; rating?: string; imdbRating?: number; director?: string; cast?: string[]; tags?: string[]; posterUrl?: string; backdropUrl?: string; trailerUrl?: string; status: string; featured?: boolean; trending?: boolean; viewCount?: number; availableQualities?: string[]; genres?: Genre[]; series?: SeriesInfo; seasons?: Season[]; mediaFiles?: MediaFile[]; createdAt: string; updatedAt: string; }
...
class ApiClient {
  ...
  private isPublicCacheable(endpoint: string) {
    // Never persist account-specific API responses in shared browser storage.
    return !endpoint.startsWith('/api/auth/') && !endpoint.startsWith('/api/watch/');
  }

  async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    const url = `${this.baseUrl}${endpoint}`;
    const method = (options.method || 'GET').toUpperCase();
    const cacheable = method === 'GET' && this.isPublicCacheable(endpoint);
    ...
    try {
      ...
      if (cacheable && typeof window !== 'undefined') await cacheResponse(url, result);
      return result;
    } catch (error) {
  ...
  async flushOfflineQueue() {
    ...
    for (const item of queued) {
      try {
        await this.request(item.endpoint, { method: item.method, body: item.body });
        await removeQueuedRequest(item.id);
      } catch {
        // Keep failed operations queued for the next online attempt.
      }
    }
  }

  async register(data: { email: string; username: string; password: string; displayName?: string }) {
    const result = await this.request<{ user: User; accessToken: string; refreshToken: string }>('/api/auth/register', { method: 'POST', body: JSON.stringify(data) });
    this.accessToken = result.accessToken;
    return result;
  }

  async login(email: string, password: string) {
    const result = await this.request<{ user: User; accessToken: string; refreshToken: string }>('/api/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) });
    this.accessToken = result.accessToken;
    return result;
  }
  ...
  async getSearchSuggestions(query: string) { return this.request<{ suggestions: Movie[] }>(`/api/search/suggestions?q=${encodeURIComponent(query)}`); }
  ...
  async updateWatchProgress(data: { movieId?: number; episodeId?: number; position: number; duration: number }) {
    try {
      return await this.request<{ success: boolean }>('/api/watch/progress', { method: 'POST', body: JSON.stringify(data) });
    } catch (error) {
  ...
  async getWatchlist() { return this.request<{ watchlist: Array<{ id: number; addedAt: string; movie: Movie }> }>('/api/watch/watchlist'); }
  async addToWatchlist(movieId: number) { return this.request<{ success: boolean }>('/api/watch/watchlist', { method: 'POST', body: JSON.stringify({ movieId }) }); }
  async removeFromWatchlist(movieId: number) { return this.request<{ success: boolean }>(`/api/watch/watchlist/${movieId}`, { method: 'DELETE' }); }
}
